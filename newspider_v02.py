...
class newspider(scrapy.Spider):
    ...
    def parse(self, response):
        for link in response.xpath('//div[@class="st-text"]/table/tr/td/a/@href').extract():
            yield response.follow(link, callback=self.parse_make_details)
               
        model = response.xpath('//div[@class="makers"]/ul/li/a/strong/span/text()').extract_first()
        yield {
            'Model' : model
        }
    ...
    def parse_make_details(self, response):
        make = response.xpath('//div[@class="st-text"]/table/tr/td/a/@href').extract_first()
        yield {
            'Make' : make  
        }
